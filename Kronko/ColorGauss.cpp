...
Vec3i ColorGauss::calcGaussAtPx(Mat & img, Mat1d & kernel, Point p) {
    ...
    if (ColorPicker::isInBounds(imgsize,p,kernelsize) ) {
        for (int x = 0 ; x < kernel.rows; x++)
        {
            ...
            {
                throw std::runtime_error("Couldn't access kernel at position: " + std::to_string(x));
            }
        ...
        throw std::runtime_error("Index out of bounds. Kernel size:" + std::to_string(kernelsize));
    }
