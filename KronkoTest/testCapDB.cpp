...
TEST(TestDB,TestCapDBNoFatelFail) {
	...
	JsonDB db;
	Cap cap = Cap::Cap(0, "Black", TEST_IMG_BLACK_PATH, cv::Vec3i(0, 0, 0), cv::Vec2b(0, 0), img, 0, 0);
	EXPECT_NO_FATAL_FAILURE(db = JsonDB("./db.json"));
...
TEST(TestDB,TestCapDBNoExep) {
	...
	JsonDB db;
	Cap cap = Cap::Cap(0, "Black", TEST_IMG_BLACK_PATH, cv::Vec3i(0, 0, 0), cv::Vec2b(0, 0), img, 0, 0);
	EXPECT_NO_THROW(db = JsonDB("./db.json"));
...
TEST(TestDB,TestCapDBStoreSuccess) {
	...
	ColorPoint colorpoint;
	JsonDB db("./db.json");
	Cap cap = Cap(0, "Black", TEST_IMG_BLACK_PATH, cv::Vec3i(0, 0, 0), cv::Vec2b(0, 0), img, 0, 0);
	db.clearDB();
	db.storeCap(cap);
